...
var LiWangNews = {
    ...
    updatePanel: function (pages, category, chind) {
        ...
        $.get(`http://test.tron-m.com/lynwon-api/news/list.do?page=${pages ? pages : 1}&rows=13&category=${category ? category : 'enlwxw'}${category == 'enhyzx' ? `&chdCategory=${chind ? chind : 'enlc'}` : ''}`, function (res) {
            if (res) {
                layer.closeAll()
            }
            ;
    ...
    updatePanel3: function (id, name) {
        ...
        $.get(`http://test.tron-m.com/lynwon-api//news/navigation.do?id=${id}&category=${name}&chdCategoryName=enpp`, function (res) {
            if (res) {
                layer.closeAll()
            }
            ;
    ...
    next: function (v) {
        ...
        if (v.next) {
            _next = `<a href="javascript:;"data-id=${v.next.id} data-name=${v.next.categoryName}>previous：${v.next.title}</a>`
        }
        _next += ` <a href="javascript:;" data-id=${v.prev.id} data-name=${v.prev.categoryName}>next：${v.prev.title}</a>`
        $('.newsNav').html(_next);
    },
    detailsHtml: function (v) {
        var _html = `<div class='col-md-8 col-md-offset-1 about-container-right'>
                    <div class="tittle">
                        <p class="left">${v.categoryName == 'enlwxw' ? 'Li Wang News' : 'Industry News'} <img src="images/1F6133389107D7F690B8AEB7E95C3731.png"
                                                  style="height: 20px;width: 20px"/></p>
                        <p class="right">your location:en>${v.categoryName == 'enlwxw' ? 'Li Wang News' : 'Industry News'}</p>
                    </div>
                    <div class="VideoCenter">
                        <div>
                            <b class="VideoCenterTitle">&nbsp;&nbsp;${v.title}</b>
                            <span class="browse">preview:${v.showCount}</span>
                            <p class="Release" >Release time: ${format(v.insertTime)}</p>
                        </div>
    ...
    selectNews: function () {
        ...
        $('.aboutNav').click(function () {
            var _name = $(this).find('a').attr('data-name');
            _me.updatePanel('', 'enhyzx', _name);
            LiWangNews.EventListening('', '', 'enhyzx');
